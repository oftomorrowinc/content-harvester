...
const addUrls = async (req, res) => {
  try {
    ...
    // Process each URL
    const results = await Promise.all(extractedUrls.map(url => contentModel.processUrl(url)));
...
const processAllContent = async (req, res) => {
  try {
    ...
    await Promise.all(
      pendingContent.map(item => contentModel.updateContentStatus(item.id, 'processing'))
    );
    ...
    setTimeout(async () => {
      await Promise.all(
        pendingContent.map(item => contentModel.updateContentStatus(item.id, 'completed'))
      );
    }, 3000);
